/**
 * Sync leads from the Meta (Facebook/Instagram) lead-form Google Sheet into the CRM.
 *
 * Every row of the sheet becomes a normal `new` lead, created by — and parked
 * on — a dedicated "Meta Ads" account, so an admin can hand each one to a sales
 * exec with the usual reassignment action. Only the columns the CRM actually
 * uses are mapped onto lead fields; the campaign/ad metadata is folded into the
 ...
 *
 * Idempotent: rows are keyed on the Meta lead id (`metaLeadId`), so the script
 * can be re-run — or left running on an interval — and only ever adds rows it
 * has not imported before. Meta's `<test lead: …>` rows are ignored, and a row
 * whose phone number already belongs to another lead is reported and skipped
 * (override with --allow-duplicate-phone).
 *
 * The sheet is read through Google's CSV export endpoint, so it must be
 * readable without a login: File -> Share -> General access -> "Anyone with the
 * link" (Viewer). No API key or service account needed.
 *
 * Usage:
 *   node src/scripts/sync-meta-leads.js                        # dry run, prints what it would import
 *   node src/scripts/sync-meta-leads.js --apply                # import new rows once
 *   node src/scripts/sync-meta-leads.js --apply --interval=15  # keep running, re-check every 15 min
 *   node src/scripts/sync-meta-leads.js --apply --file=leads.csv   # import a downloaded CSV instead
 *
 * Options:
 ...
 *
 * Env equivalents: META_SHEET_ID, META_SHEET_GID, META_SHEET_CSV_URL.
 */
...
// The Meta Ads lead-form sheet this sync was built against.
...
const ALLOW_DUP_PHONE = Boolean(ARGS['allow-duplicate-phone']);
const INTERVAL_MIN = Number(ARGS.interval) || 0;

function sourceUrl() {
  if (ARGS.url) return String(ARGS.url);
  if (process.env.META_SHEET_CSV_URL) return process.env.META_SHEET_CSV_URL;
  const id = ARGS.sheet || process.env.META_SHEET_ID || DEFAULT_SHEET_ID;
  const gid = ARGS.gid || process.env.META_SHEET_GID || '0';
  ...
  if (/^\s*</.test(body)) {
    throw new Error(
      'the sheet is not publicly readable — open it, then Share -> General access -> ' +
        '"Anyone with the link" (Viewer), or pass --file=<downloaded csv>'
    );
  }
  return body;
}
...
async function sync() {
  ...
  if (!records.length) {
    console.log('[meta-sync] no data rows found in the sheet');
    return;
  }

  const metaUser = await getMetaUser();
  const existingIds = new Set(
    (await Lead.find({ metaLeadId: { $nin: ['', null] } }).select('metaLeadId').lean())
      .map((l) => l.metaLeadId)
  );
  ...
  for (const rec of records) {
    ...
    if (key && phoneIndex.has(key) && !ALLOW_DUP_PHONE) {
      stats.dupPhone += 1;
      console.log(`  ~ ${mapped.businessName} (${mapped.mobileNumber}) — already on ${phoneIndex.get(key)}, skipped`);
      continue;
    }
    ...
    if (!APPLY) {
      console.log(`  + ${mapped.businessName} · ${mapped.contactPerson} · ${mapped.city} · ${mapped.businessType}`);
      continue;
    }
    ...
    console.log(`  + ${lead.refNumber} · ${lead.businessName} · ${lead.city} · ${lead.businessType}`);
  }

  console.log(
    `[meta-sync] ${stats.rows} row(s): ${stats.imported} ${APPLY ? 'imported' : 'would import'}, ` +
      `${stats.existing} already imported, ${stats.dupPhone} duplicate phone, ${stats.skipped} not a lead`
  );
}
...
async function run() {
  ...
  const tick = async () => {
    try {
      ...
      console.error(`[meta-sync] ${err.message}`);
    }
...
// Exported so the row mapping can be exercised without a database.
module.exports = { parseCsv, toRecords, toLead, mapBusinessType, cleanAnswer, cleanPhone, tidy };
